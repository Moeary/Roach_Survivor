...
React + TypeScript + Vite 重构版的蟑螂幸存者原型。当前版本加入了难度轮盘、怪物图鉴、音量控制、统一金色卵鞘掉落、10 种敌人梯度、局外成长，以及分阶段音频资源入口。
...
- 主页新增怪物图鉴，可以直接查看当前 SVG 怪物形象以及不同难度下实际生效的血量、伤害、速度。
- 主页新增音量调整，可分别控制全体、BGM 和 SFX 音量。
- 普通怪的金色卵鞘掉率改为按难度统一配置，当前三档先统一为 `0.2%`，Boss 波次会固定掉落更多金色卵鞘。
- 敌人池按时间扩展：`0-5` 分钟 3 种、`5-10` 分钟新增 3 种、`10-15` 分钟再新增 4 种。
- 简单难度保持基础怪物强度；普通为生命/伤害 `1.5x`、速度 `1.2x`；困难为生命/伤害 `2x`、速度 `1.4x`。
- 第二波 Boss 改为高速冲阵型，第三波 Boss 改为会召唤小弟的最终 Boss。
